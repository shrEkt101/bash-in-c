...
int find_network_newline(const char *buf, int inbuf) {
    for (int i=0; i< inbuf;i++){
        if(buf[i]=='\r' && buf[i+1]=='\n'){
            // return one characyer after newline
            return i+2;
        }
    }
    return -1;
...
int read_from_socket(int sock_fd, char *buf, int *inbuf) {
    int max = BUF_SIZE-*inbuf;
    int have_read = read(sock_fd,buf+*inbuf, max);
    ...
    *inbuf += have_read;
    ...
    if(find_network_newline(buf, *inbuf)){
        return 0;
    }
...
int get_message(char **dst, char *src, int *inbuf) {
    ...
    // Find the index of the network newline (CRLF) in the buffer.
    int msg_len = find_network_newline(src, *inbuf);

    if (msg_len == -1) {
        // If no network newline is found, return an error.
        return 1;
    }

    // Allocate memory for the complete message, which includes the CRLF
    // plus the null-terminator for the string.
    *dst = malloc(msg_len + 1);  // Allocate memory for message + null terminator
    if (*dst == NULL) {
        perror("malloc");
        return 1;
    }

    // Copy the complete message (including CRLF) into the newly allocated string.
    memcpy(*dst, src, msg_len);

    // Null-terminate the copied message.
    (*dst)[msg_len] = '\0';

    // Shift the remaining content of the buffer to the front, removing the extracted message.
    memmove(src, src + msg_len, *inbuf - msg_len);  // Shift the remaining bytes in the buffer

    // Update the byte count in the buffer.
    *inbuf -= msg_len;
