...
def _test_1unknown_command(comment_file_path, student_dir, timeout=TESTS_TIMEOUT):
  ...
  try:
    ...
    if "ERROR: Unknown command: destroy" in output and "input line too long" not in output:
      finish(comment_file_path, "OK", p)
    else:
      finish(comment_file_path, "NOT OK", p)
  ...
  try:
    ...
    for i in range(10):
        ...
        if "ERROR: Unknown command" not in read_stderr(process):
          finish(comment_file_path, "NOT OK", process)
          return
    else:
      write(process, "exit")
      finish(comment_file_path, "OK", process)
  except Exception as e:
    finish(comment_file_path, "NOT OK", process)

def _test_short_line(comment_file_path, student_dir, timeout=TESTS_TIMEOUT):
...
def _test_long_line(comment_file_path, student_dir, timeout=TESTS_TIMEOUT):
...
def _test_long_priority(comment_file_path, student_dir, timeout=TESTS_TIMEOUT):
