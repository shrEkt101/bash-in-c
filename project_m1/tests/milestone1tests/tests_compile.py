...
from tests_helpers import TESTS_TIMEOUT
...
def _test_compiles(student_dir, comment_file):
  ...
  try:
    output = check_output(["rm", "*.o mysh"], stderr=STDOUT, timeout=TESTS_TIMEOUT) 
  except CalledProcessError:
    pass    # No .o files, ignore errors
  
  try:
    output = check_output(["make", "clean"], stderr=STDOUT, timeout=TESTS_TIMEOUT) 
  except CalledProcessError:
    pass   # Nothing to clean, ignore errors

  try:
    output = check_output(["make", "-n"], stderr=STDOUT, timeout=TESTS_TIMEOUT) 

    if not (b'Wall' in output and b'Werror' in output and b'Wextra' in output \
       and b'fsanitize' in output and b'address' in output):
      comment_file.write("FAIL\n")
      return False

    output = check_output(["make"], stderr=STDOUT, timeout=TESTS_TIMEOUT)
